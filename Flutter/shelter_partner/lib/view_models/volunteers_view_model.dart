...
class VolunteersViewModel extends StateNotifier<AsyncValue<Shelter?>> {
  ...
  VolunteersViewModel(this._repository, this.ref) : super(const AsyncValue.loading()) {
...
Future<void> modifyVolunteerSettingString(String shelterID, String field, String newValue) async {
  try {
    await _repository.modifyVolunteerSettingString(shelterID, field, newValue);
  } catch (error) {
...
final volunteersViewModelProvider = StateNotifierProvider<VolunteersViewModel, AsyncValue<Shelter?>>((ref) {
  final repository = ref.watch(volunteersRepositoryProvider); // Access the repository
