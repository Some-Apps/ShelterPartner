...
//    var prioritizeMinutes: Int
    ...
    var playtimeLast90Days: Int {
        let nintyDaysAgo = Calendar.current.date(byAdding: .day, value: -90, to: Date())!
        return logs.filter { Date(timeIntervalSince1970: $0.startTime) >= nintyDaysAgo }
            .reduce(0) { $0 + $1.durationInMinutes }
    }
